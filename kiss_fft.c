        ...
        )
{
    ...
    tw3 = tw2 = tw1 = st->twiddles; // tw2 is just tw1^2, tw3=tw1^3=tw1*tw2

    // Hoist st->inverse check
    const size_t m_inv = st->inverse ? 3*m : m;
    const size_t m3_inv = st->inverse ? m : 3*m;

    do {
        C_FIXDIV(*Fout,4); C_FIXDIV(Fout[m],4); C_FIXDIV(Fout[m2],4); C_FIXDIV(Fout[m3],4);

        C_MUL(scratch[0],Fout[m] , *tw1 );
        C_MUL(scratch[1],Fout[2*m] , *tw2 );
        C_MUL(scratch[2],Fout[3*m] , *tw3 );

        C_SUB( scratch[5] , *Fout, scratch[1] );
        C_ADDTO(*Fout, scratch[1]);
        C_ADD( scratch[3] , scratch[0] , scratch[2] );
        C_SUB( scratch[4] , scratch[0] , scratch[2] );
        C_SUB( Fout[2*m], *Fout, scratch[3] );
        tw1 += fstride;
        tw2 += fstride*2;
        tw3 += fstride*3;
        C_ADDTO( *Fout , scratch[3] );

        Fout[m_inv].r = scratch[5].r + scratch[4].i;
        Fout[m_inv].i = scratch[5].i - scratch[4].r;
        Fout[m3_inv].r = scratch[5].r - scratch[4].i;
        Fout[m3_inv].i = scratch[5].i + scratch[4].r;
